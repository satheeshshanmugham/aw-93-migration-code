...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:MadCap="http://www.madcapsoftware.com/Schemas/MadCap.xsd" exclude-result-prefixes="xs"
    version="2.0">
    ...
    <xsl:template match="//TocEntry/@BreakType"/>
    <xsl:template match="//TocEntry/@PageType"/>
    <xsl:template match="//TocEntry/@PageNumber"/>
    <xsl:template match="//TocEntry/@SyncDocumentTitle"/>
    ...
    <xsl:template match="//TocEntry">
        <xsl:param name="load_file" select="document(concat('.', @Link))"/>
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
